    ...
    useEffect(() => {
      const removeBackListener = on('back_button_pressed', () => {
        if (isEditMode && pushId) {
          // «страница-деталь пуша»; не кладём текущую в history
          navigate(`/moderator/pushes/${pushId}`, { replace: true });
        } else {
          // «список пушей»; тоже без добавления в стек
          navigate('/moderator/pushes', { replace: true });
        }
      });

      return removeBackListener;        // корректный cleanup
