...
import {createOrder, getProductById, getUserOrders} from '../../services/api';
...
function translatePaymentTime(value: string): string {
    switch (value) {
        ...
        case 'ON_15TH_DAY':
            return '— через 15 дней после получения товара';
        default:
            return value;
    }
}
...
const InstructionPage: React.FC = () => {
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            const existing = res.data.find((o: any) =>
  matchesProduct(o, productId) &&
  ...
  <strong>@bigblacklist_bot</strong>
                ...
                {!preview && (
                    <div className="space-y-4">
                        ...
                        {/* agreeRules */}
                        <label htmlFor="agreeRules" className="flex items-start gap-3 text-sm cursor-pointer">
                            <input
                                id="agreeRules"
                                type="checkbox"
                                checked={agreeRules}
                            ...
                            <span
                                aria-hidden
                                className="
          mt-1 inline-grid place-items-center
          h-5 w-5 rounded-md border border-gray-400 bg-white
          transition-colors
        ...
        >
          <polyline points="20 6 10 16 5 12"/>
        </svg>
      </span>
        ...
        <ul className="list-disc ml-6 mt-2 space-y-1">
          ...
          <li>беру на себя ответственность за проверку продавца и соблюдение всех условий.</li>
                        ...
                        {/* agreeData */}
                        <label htmlFor="agreeData" className="flex items-center gap-3 text-sm cursor-pointer">
                            <input
                                id="agreeData"
                                type="checkbox"
                                checked={agreeData}
