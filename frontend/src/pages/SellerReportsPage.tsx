...
import {getOrderBySellerId} from '../services/api';
...
function SellerReportsPage() {
    ...
    const filteredOrders = orders
        .filter(o => {
            if (activeTab === OrderStatus.CASHBACK_NOT_PAID) {
                // во "выплатить" не попадают отклонённые
                return o.status === OrderStatus.CASHBACK_NOT_PAID;
            }
            // во "выплачен" попадают и выплаченные, и отклонённые
            return (
                o.status === OrderStatus.CASHBACK_PAID ||
                o.status === (OrderStatus as any).CASHBACK_REJECTED // ← используем ваше значение енума
            );
        })
        .filter(o => {
            ...
            const dl = getDaysLeft(o); // может быть null
        ...
        >
          ...
          <div className="flex gap-3 p-2">
            ...
            <div className="flex-1">
              <h2 className={`text-lg font-semibold ${isRejected ? "text-red-700" : ""}`}>
                {order.product.name}
              </h2>
              {isRejected ? (
